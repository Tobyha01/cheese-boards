...
describe("User, Board and Cheese test suite", function() {
    ...
    test("Check Board-Cheese association", async function() {
        ...
        await board1.addCheeses([1,2])
        await board2.addCheeses([1])
        const cheeseBoard1 = await Board.findByPk(1, {include: [{model: Cheese, as: "Cheeses"}]})
        const cheeseBoard2 = await Board.findByPk(2, {include: [{model: Cheese, as: "Cheeses"}]})
        expect(cheeseBoard1.Cheeses[0].title).toBe("Brie")
        expect(cheeseBoard1.Cheeses[1].title).toBe("Edam")
        expect(cheeseBoard1.Cheeses[0].description).toBe("Soft")
        expect(cheeseBoard2.Cheeses[0].title).toBe("Brie")
        expect(cheeseBoard2.Cheeses[0].description).toBe("Soft")
    })
})